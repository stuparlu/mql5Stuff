...
#define EXIT_BUFFER_INDEX 0
string GetExitSignal()    // For zero cross or one line indies
{
//---
   int candleNumber = 2;
   double signalLine[]; 
   FillArrayFromBuffer(signalLine, entryHandle, candleNumber , EXIT_BUFFER_INDEX);
   //double signalValue = signalLine[1];
  // double signalBefore = signalLine[0];
   bool buyable = false;
   bool sellable = false;
   ...
   for (int i = candleNumber - 1; i > 0; i--) 
   {
      if (signalLine[i] > 70 && signalLine[i - 1] <= 70) 
      {
         sellable = true;
         break;
      }
      
      if (signalLine[i] < 30 && signalLine[i - 1] >= 30)
      {
         buyable = true;
         break;
      }
